...
@section('page-subtitle', 'Update venue information and settings')
...
@section('header-actions')
    <div class="flex items-center space-x-3">
        <a href="{{ route('mhadel.venues.show', $venue) }}" 
           class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all duration-300 flex items-center shadow-lg font-bold">
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="bg-maroon rounded-xl shadow-lg overflow-hidden">
        <div class="p-6 text-white">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-3xl font-bold font-poppins mb-2">Edit Venue</h1>
                    <p class="text-lg text-red-100 font-inter">Update information for {{ $venue->name }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100">
        <form action="{{ route('mhadel.venues.update', $venue->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div id="equipment-container" class="space-y-3">
                        @if($venue->available_equipment)
                            @foreach($venue->available_equipment as $index => $equipment)
                                <div class="equipment-item bg-gray-50 rounded-lg border border-gray-200 p-4 hover:shadow-md transition-all duration-300">
                                    <div class="flex items-center space-x-3">
                                        <div class="flex-1">
