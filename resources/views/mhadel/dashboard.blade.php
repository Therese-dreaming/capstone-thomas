...
<div class="space-y-6 font-poppins">
	...
	<div class="bg-white border border-gray-100 rounded-xl shadow-md p-6">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-2xl font-bold text-gray-800 font-montserrat mb-1">Welcome, {{ Auth::user()->name }}!</h1>
				<p class="text-gray-600">You are the second level of approval for reservations. Review IOSA-approved reservations and make final decisions.</p>
			</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
		...
		<div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 flex items-center">
			...
			<div>
				...
				<p class="text-xs text-gray-500">Forwarded to OTP</p>
			</div>
		</div>
	...
	<div class="bg-white rounded-xl shadow-sm border border-gray-100">
		<div class="p-6 border-b border-gray-200">
			<h2 class="text-xl font-bold text-gray-800 font-montserrat flex items-center">
				<i class="fas fa-bolt text-gray-700 mr-3"></i>
				Quick Actions
			</h2>
		</div>
		<div class="p-6">
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				<a href="{{ route('mhadel.reservations.index') }}" class="bg-gray-800 text-white p-6 rounded-xl hover:bg-gray-900 transition-colors shadow-sm">
					<div class="flex items-center">
						<i class="fas fa-calendar-check text-3xl mr-4"></i>
						<div>
							<h3 class="text-lg font-semibold">Review Reservations</h3>
							<p class="text-white/80 text-sm">View and manage pending reservations</p>
						</div>
					</div>
				</a>
				
				<a href="{{ route('mhadel.venues.index') }}" class="bg-blue-600 text-white p-6 rounded-xl hover:bg-blue-700 transition-colors shadow-sm">
					<div class="flex items-center">
						<i class="fas fa-building text-3xl mr-4"></i>
						<div>
							<h3 class="text-lg font-semibold">Manage Venues</h3>
							<p class="text-white/80 text-sm">Create and manage venues</p>
						</div>
					</div>
				</a>
				
				<a href="{{ route('mhadel.events.index') }}" class="bg-green-600 text-white p-6 rounded-xl hover:bg-green-700 transition-colors shadow-sm">
					<div class="flex items-center">
						<i class="fas fa-calendar-alt text-3xl mr-4"></i>
						<div>
							<h3 class="text-lg font-semibold">Manage Events</h3>
							<p class="text-white/80 text-sm">Create and manage events</p>
						</div>
					</div>
				</a>
			</div>
	...
	<div class="bg-white rounded-xl shadow-sm border border-gray-100">
		<div class="p-6 border-b border-gray-200">
			<div class="flex items-center justify-between">
				...
				<a href="{{ route('mhadel.reservations.index') }}" class="text-gray-800 hover:text-black font-medium">
					...
					@foreach($recent_reservations as $reservation)
						<div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
							...
							<div class="flex items-center space-x-2">
								...
								<button onclick="openApproveModal({{ $reservation->id }}, '{{ $reservation->event_title }}')" class="p-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors" title="Approve">
									<i class="fas fa-check"></i>
								</button>
								<button onclick="openRejectModal({{ $reservation->id }}, '{{ $reservation->event_title }}')" class="p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors" title="Reject">
