...
<script>
	...
	function openDetails(dateStr){
			...
			wrap.insertAdjacentHTML('beforeend', `<div class="bg-blue-50 p-4 rounded-lg mb-4 border border-blue-200">
				<div class="flex items-start justify-between">
					...
					<div class="flex items-center space-x-2">
						...
						<a href="/mhadel/events/${e.id}/edit" 
						   class="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" title="Edit Event">
		...
		dayReservations.forEach(e=>{
			...
			const canEdit = !['rejected_IOSA', 'rejected_mhadel', 'rejected_OTP', 'cancelled'].includes(e.status);
			...
			wrap.insertAdjacentHTML('beforeend', `<div class="bg-gray-50 p-4 rounded-lg mb-4 border border-gray-200">
				<div class="flex items-start justify-between">
					...
					<div class="flex items-center space-x-2">
						...
						${canEdit ? `<a href="/mhadel/reservations/${e.id}/edit" 
								class="p-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors" title="Edit Reservation">
							<i class="fas fa-edit text-sm"></i>
						</a>` : ''}
					</div>
				</div>
						...
						html += `<div class="p-1 rounded border text-xs cursor-pointer hover:shadow-md transition-shadow mb-1 last:mb-0" 
							...
							<div class="h-full flex flex-col justify-center">
								...
								<div class="mt-1 flex items-center justify-between">
									...
									${isEvent ? 
										`<a href="/mhadel/events/${item.data.id}/edit" class="ml-1 p-0.5 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 transition-colors" title="Edit Event">
											<i class="fas fa-edit"></i>
										</a>` : 
										!['rejected_IOSA', 'rejected_mhadel', 'rejected_OTP', 'cancelled'].includes(item.data.status) ?
										`<a href="/mhadel/reservations/${item.data.id}/edit" class="ml-1 p-0.5 bg-amber-600 text-white rounded text-xs hover:bg-amber-700 transition-colors" title="Edit Reservation">
											<i class="fas fa-edit"></i>
										</a>` : ''
									}
								</div>
