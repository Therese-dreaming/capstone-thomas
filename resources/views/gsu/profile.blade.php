    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
            ...
            <div class="bg-maroon p-6 text-white">
                <div class="flex flex-col lg:flex-row items-center lg:items-start space-y-4 lg:space-y-0 lg:space-x-6">
                    ...
                    <div class="flex-1 text-center lg:text-left">
                        <h1 class="text-2xl font-bold mb-1">{{ $user->name ?? 'GSU Staff' }}</h1>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 w-full lg:w-auto">
                        ...
                        <div class="text-center p-3 bg-white/10 rounded-lg border border-white/20">
                            <div class="text-lg font-bold">{{ $user->events_count ?? 0 }}</div>
                            <div class="text-xs opacity-90">Events</div>
                        </div>
                        <div class="text-center p-3 bg-white/10 rounded-lg border border-white/20">
                            <div class="text-lg font-bold">{{ $user->issues_resolved ?? 0 }}</div>
                            <div class="text-xs opacity-90">Issues Resolved</div>
                        </div>
                        <div class="text-center p-3 bg-white/10 rounded-lg border border-white/20">
                            <div class="text-lg font-bold">{{ $user->pending_tasks ?? 0 }}</div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            ...
            <div id="content-account" class="tab-content active">
                ...
                <form method="POST" action="{{ route('gsu.profile.update') }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="first_name" value="{{ old('first_name', $user->first_name ?? '') }}" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon focus:border-transparent" />
                            @error('first_name')
                    ...
                    <div>
                        ...
                        <input type="text" name="department" value="{{ old('department', $user->department ?? 'General Services Unit') }}" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon focus:border-transparent" />
                        @error('department')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div id="content-activity" class="tab-content">
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-600">View your completion statistics and recent activity</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div class="text-center p-4 bg-blue-600 rounded-lg text-white">
                        ...
                        <div class="text-sm text-blue-100">Reservations Completed</div>
                    </div>
                    ...
                    <div class="text-center p-4 bg-green-600 rounded-lg text-white">
                        ...
                        <div class="text-2xl font-bold text-white mb-1">{{ $user->events_count ?? 0 }}</div>
                        <div class="text-sm text-green-100">Events Managed</div>
                    </div>
                    
                    <div class="text-center p-4 bg-purple-600 rounded-lg text-white">
                        <div class="w-12 h-12 bg-purple-500 rounded-full flex items-center justify-center mx-auto mb-2">
                            <i class="fas fa-tools text-white text-xl"></i>
                        </div>
                        <div class="text-2xl font-bold text-white mb-1">{{ $user->issues_resolved ?? 0 }}</div>
                        <div class="text-sm text-purple-100">Issues Resolved</div>
                    </div>
                    ...
                    <div class="text-center p-4 bg-yellow-600 rounded-lg text-white">
                        ...
                        <div class="text-2xl font-bold text-white mb-1">{{ $user->pending_tasks ?? 0 }}</div>
                        <div class="text-sm text-yellow-100">Pending Tasks</div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    @php
                        $recentReservations = \App\Models\Reservation::where('status', 'completed')
                    ...
                    @if($recentReservations->count() > 0)
                        <div class="space-y-4">
                            @foreach($recentReservations as $reservation)
                                <div class="bg-gray-50 rounded-xl p-6 border-l-4 border-indigo-500 hover:bg-gray-100 transition-colors duration-200">
                        ...
                        <div class="text-center mt-6">
                            <a href="{{ route('gsu.reservations.index') }}" 
                               class="inline-flex items-center px-6 py-3 bg-maroon text-white rounded-xl hover:bg-red-800 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href="{{ route('gsu.dashboard') }}" class="group bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-200 transform hover:-translate-y-1">
                    <div class="flex items-center">
                ...
                <a href="{{ route('gsu.reservations.index') }}" class="group bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-200 transform hover:-translate-y-1">
                    <div class="flex items-center">
                        <div class="w-12 h-12 bg-maroon rounded-xl flex items-center justify-center mr-4 group-hover:scale-110 transition-transform duration-200">
                            <i class="fas fa-calendar-check text-white text-xl"></i>
                        </div>
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Manage reservation completion</p>
                        </div>
                    </div>
                </a>
                
                <a href="{{ route('gsu.events.index') }}" class="group bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-200 transform hover:-translate-y-1">
                    <div class="flex items-center">
                        <div class="w-12 h-12 bg-maroon rounded-xl flex items-center justify-center mr-4 group-hover:scale-110 transition-transform duration-200">
                            <i class="fas fa-calendar-alt text-white text-xl"></i>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-800 mb-1">Events</h3>
                            <p class="text-sm text-gray-600">View and manage events</p>
                        </div>
                    </div>
                </a>
            </div>
...
<script>
    function showTab(name) {
        ...
        // Hide all tab panels using both class and inline style
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('DOM loaded, setting up floating labels');
        ...
        document.getElementById('tab-account').classList.add('bg-maroon', 'text-white', 'shadow-md');
        document.getElementById('tab-account').classList.remove('text-gray-600', 'hover:bg-gray-200');
        
        // Form inputs now use standard label-above-input design
    });
</script>
@endpush
