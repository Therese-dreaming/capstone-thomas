...
<div class="max-w-4xl mx-auto space-y-6 font-poppins profile-page">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center mt-2">
                            ...
                            <span class="ml-2 text-xs text-white/80">IOSA Member since {{ $user->created_at->format('M Y') }}</span>
                        </div>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div id="content-account" class="tab-content active space-y-6">
            ...
            <form method="POST" action="{{ route('iosa.profile.update') }}" class="space-y-5">
                @csrf
                @method('PUT')
        ...
        <div id="content-security" class="tab-content space-y-6">
            ...
            <form method="POST" action="{{ route('iosa.password.update') }}" class="space-y-5">
                @csrf
                @method('PUT')
        ...
        <div id="content-stats" class="tab-content space-y-6">
            <div class="border-b border-gray-200 pb-4 mb-4">
                <h2 class="text-xl font-bold text-gray-800 font-montserrat">IOSA Statistics</h2>
                <p class="text-gray-600 text-sm">Overview of your IOSA activities and system metrics</p>
            </div>

            @php
                $totalReservations = \App\Models\Reservation::count();
                $pendingReservations = \App\Models\Reservation::whereIn('status', ['pending', 'approved_IOSA'])->count();
                $approvedReservations = \App\Models\Reservation::whereIn('status', ['approved_mhadel', 'approved_OTP'])->count();
                $rejectedReservations = \App\Models\Reservation::whereIn('status', ['rejected_IOSA', 'rejected_mhadel', 'rejected_OTP'])->count();
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="text-center p-4 bg-red-50 rounded-lg border border-red-100 stat-card">
                    ...
                    <div class="text-2xl font-bold text-red-600">{{ $rejectedReservations }}</div>
                    <div class="text-sm text-gray-600">Rejected</div>
                </div>
                <div class="text-center p-4 bg-purple-50 rounded-lg border border-purple-100 stat-card">
                ...
                <div class="text-center p-4 bg-indigo-50 rounded-lg border border-indigo-100 stat-card">
                    <div class="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-2">
                        <i class="fas fa-users text-indigo-600 text-xl"></i>
                    </div>
                    <div class="text-2xl font-bold text-indigo-600">{{ $totalUsers }}</div>
            ...
            <div class="mt-6">
                ...
                @php
                    $recentReservations = \App\Models\Reservation::with(['user', 'venue'])->latest()->take(5)->get();
                @endphp
                ...
                @if($recentReservations->count() > 0)
                                            ...
                                            @switch($reservation->status)
                                                ...
                                                @case('approved_IOSA')
                                                    <span class="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                                                        <i class="fas fa-check mr-1"></i> IOSA Approved
                                                    </span>
                                                    @break
                                                @case('approved_mhadel')
                                                    <span class="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">
                                                        <i class="fas fa-check-circle mr-1"></i> Mhadel Approved
                                                    </span>
                                                    @break
                                                @case('approved_OTP')
                                                    <span class="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">
                    ...
                    <div class="mt-4 text-center">
                        <a href="{{ route('iosa.reservations.index') }}" class="inline-block text-sm text-maroon hover:text-red-800">
                ...
                @else
                    <div class="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                        ...
                        <p class="text-gray-600">No reservations in the system yet</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <a href="{{ route('iosa.reservations.index') }}" class="flex items-center p-3 bg-maroon text-white rounded-lg hover:bg-red-800 transition-colors action-card">
                <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center mr-3">
                    <i class="fas fa-calendar-check"></i>
                </div>
                <div>
                    <h3 class="font-medium">Manage Reservations</h3>
                    <p class="text-sm opacity-80">Review and approve reservations</p>
                </div>
            </a>
            
            <a href="{{ route('iosa.events.index') }}" class="flex items-center p-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors action-card">
                <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center mr-3">
                    <i class="fas fa-calendar-alt"></i>
                </div>
                <div>
                    <h3 class="font-medium">View Events</h3>
                    <p class="text-sm opacity-80">Browse all system events</p>
                </div>
            </a>
            
            <a href="{{ route('iosa.reservations.calendar') }}" class="flex items-center p-3 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition-colors action-card">
                <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center mr-3">
                    <i class="fas fa-calendar"></i>
                </div>
                <div>
                    <h3 class="font-medium">Calendar View</h3>
                    <p class="text-sm opacity-80">See all scheduled events</p>
                </div>
            </a>
        </div>
    </div>
